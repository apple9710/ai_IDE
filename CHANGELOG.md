...
### 변경
- **최신 모델 반영** — Claude 모델 셀렉터에 Opus 5(`claude-opus-5`, 2026-07-24 출시) 추가. 기존 `opus` 별칭은 CLI 버전에 따라 가리키는 모델이 달라져서 Opus 5/4.8 모두 명시적 ID로 전환 (저장된 `opus` 설정은 Opus 5로 자동 이관). Codex 모델 목록에 GPT-5.6 계열(Sol/Terra/Luna, 2026-07-09 출시) 반영 — 동적 목록은 CLI 캐시 갱신으로, HTML 폴백 목록도 함께 업데이트.
- **SDK 업데이트** — `@anthropic-ai/claude-agent-sdk` 0.3.198 → 0.3.220, `@openai/codex-sdk` 0.142.5 → 0.145.0 (구버전 SDK는 GPT-5.6 모델 목록을 받지 못했음).

## [0.5.0] - 2026-07-10

### 추가
- **채팅 마크다운 렌더링** — 코드블록·인라인 코드만 처리하던 `renderMd`를 제목(h1~h4), 굵게/기울임/취소선, 순서·비순서 목록(중첩 1단계, 체크박스), 인용, 표, 구분선, 링크까지 지원하도록 확장. 스트리밍 중에도 실시간으로 마크다운이 적용되고(Claude·Codex 모두), 링크는 기본 브라우저로 열림(`shell.openExternal`, http/https만). 원문을 전부 이스케이프한 뒤 태그를 입히는 방식이라 HTML 주입은 불가.

## [0.4.0] - 2026-07-10

### 추가
- **AskUserQuestion 선택지 카드** — Claude가 선택을 요청할 때(예: "어떤 방식으로 할까요?") 허용/거부 권한 카드 대신 실제 선택지가 보이는 카드로 렌더링. 옵션 클릭으로 선택(복수 선택 질문 지원), '기타'로 직접 입력 가능, 답변은 `updatedInput.answers`로 SDK에 전달됨. 기존엔 일반 권한 카드로 떠서 선택지가 아예 보이지 않았음.

## [0.3.0] - 2026-07-10

### 추가
- **파일 시스템 자동 새로고침** — 프로젝트 폴더를 재귀 감시(`fs.watch`)해서, 터미널·Claude 등 외부에서 파일이 생성/수정/삭제되면 새로고침 버튼 없이 파일 트리와 git 배지가 자동 갱신됨. 열린 에디터 탭도 디스크 내용이 바뀌면 자동 반영 (저장 안 한 편집이 있는 탭은 보존).
