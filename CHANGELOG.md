...
### 수정
- 권한 카드에서 **허용/항상 허용을 눌러도 도구 실행이 ZodError로 실패**하던 문제. allow 응답에 CLI가 필수로 요구하는 `updatedInput`(원본 도구 입력)을 빠뜨리고 있었음 — 파일 생성/수정, Bash 등 승인이 필요한 모든 작업이 막히던 원인.

## [0.2.2] - 2026-07-08

### 제거
- `request/` 요청함 폐기 — 개인용 워크플로라 저장소에서 제외 (폴더·README 삭제). 백그라운드 처리 루프도 더 이상 운용하지 않음.
